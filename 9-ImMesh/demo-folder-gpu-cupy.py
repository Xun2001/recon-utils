...
def process_image(args, depth_bins, num_bins, image_path, save_path):
    ...
    for i in range(num_bins):
        ...
        interpolator = LinearNDInterpolator(all_points_gpu, all_depths_gpu)
...
def process_folder(args, input_folder, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    image_paths = glob(os.path.join(input_folder, '*.png'))
    depth_bins = np.linspace(0, args['max_depth'], args['num_bins'] + 1)

    with ThreadPoolExecutor(max_workers=args['num_workers']) as executor:
        futures = []
        for image_path in image_paths:
            futures.append(executor.submit(process_image, args, depth_bins, args['num_bins'], image_path, output_folder))
